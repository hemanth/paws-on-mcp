...
def handle_roots_list(params: Dict[str, Any] = None) -> Dict[str, Any]:
    """Handle roots/list method per MCP 2025-03-26 specification.
    
    This is a direct method handler that returns the list of roots
    available on the server side.
    """
    try:
        # Return server-side roots according to the spec
        roots = [
            {
                "uri": "hackernews://",
                "name": "HackerNews Data Source", 
                "description": "Access to HackerNews stories, trends, and community discussions with AI analysis capabilities"
            },
            {
                "uri": "github://",
                "name": "GitHub Repository Source",
                "description": "Access to GitHub repositories, trending projects, and code analysis with AI insights"
            },
            {
                "uri": "sampling://",
                "name": "MCP Sampling Engine",
                "description": "Server-side data sampling with enhanced model preferences and multi-source analysis"
            },
            {
                "uri": "analysis://",
                "name": "AI Analysis Engine", 
                "description": "AI-powered analysis using MCP 2025-03-26 sampling with model hints and preferences"
            },
            {
                "uri": "file://workspace/",
                "name": "Workspace Root",
                "description": "File system access for document analysis and workspace operations"
            }
        ]
        ...
        return {
            ...
            "meta": {
                ...
                "serverName": "Unified MCP Server",
                "totalRoots": len(roots)
            }
        }
        
    except Exception as e:
        return {"error": f"Failed to list roots: {str(e)}"}

def request_client_roots() -> Dict[str, Any]:
    """Request roots from client per MCP 2025-03-26 specification.
    
    This demonstrates the new roots capability where servers can request
    specific directories or files to operate on from the client.
    
    Returns:
        A roots/list request that should be sent to the client
    """
    try:
        roots_request = {
            "method": "roots/list",
            "params": {
                "_meta": {
                    "protocolVersion": MCP_PROTOCOL_VERSION,
                    "requestReason": "Server needs access to file system for enhanced analysis capabilities"
                }
            }
        }
        
        return {
            "roots_request": roots_request,
            "status": "ready_for_client",
            "description": f"MCP {MCP_PROTOCOL_VERSION} roots request - send to client to get accessible directories",
            "note": "Client will respond with available file:// URIs that server can operate on"
        }
        
    except Exception as e:
        return {"error": f"Failed to create roots request: {str(e)}"}
